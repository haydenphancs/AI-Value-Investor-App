...
def _install_fakes(
    ...
    *,
    ...
    try_charge / refund / generate_fresh_report.
    """
    ...
    try_charge = MagicMock()
    if charge_raises:
        try_charge.side_effect = CreditServiceUnavailable("transient")
    else:
        try_charge.return_value = charge_return
    refund = MagicMock(return_value=80)
    ...
    FakeCreditService.try_charge = try_charge
    FakeCreditService.refund = refund
...
async def test_cache_hit_is_free(monkeypatch):
    ...
    credit.refund.assert_not_called()
...
async def test_miss_success_charges_once_no_refund(monkeypatch):
    ...
    await tr.get_ticker_report("AAPL", "warren_buffett", user=USER)
    credit.try_charge.assert_called_once()
...
async def test_miss_generation_failure_refunds_once(monkeypatch):
    ...
    await tr.get_ticker_report("AAPL", "warren_buffett", user=USER)
    credit.try_charge.assert_called_once()
    credit.refund.assert_called_once()
