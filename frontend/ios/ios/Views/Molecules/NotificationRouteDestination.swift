...
struct NotificationRouteContent: View {
    ...
    var body: some View {
        Group {
            ...
            case .report(let reportId, let ticker, let persona):
                // THE REPORT, not the ticker.
                //
                // This used to open `TickerDetailView`, so "PLUG analysis is ready — tap
                // to read the full report" did everything except read the full report.
                // The report id had been in the payload the whole time, carried "for a
                // future direct-open" that never arrived, and nothing pointed at the
                // mismatch because the tap still landed somewhere plausible.
                //
                // Fixed HERE rather than at the inbox call site so the PUSH tap is fixed
                // too — that is the entire reason this dispatcher exists.
                //
                // Costs nothing on its own: `loadReport()` gates on sign-in and a cache
                // miss raises `needsPaidRegeneration` instead of spending.
