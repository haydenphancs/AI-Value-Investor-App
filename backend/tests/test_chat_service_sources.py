...
def test_ticker_context_pill_uses_ticker_before_pipe():
    ...
    sources = ChatService._build_sources(
        "TICKER_REPORT", "AAPL|warren_buffett", None, resolved_context="report grounding",
    )
    assert sources == [{"label": "Cay research report", "detail": "AAPL"}]

    sources = ChatService._build_sources("STOCK", "MSFT", None, grounded=True)
...
def test_slug_context_has_no_ticker_detail():
    ...
    sources = ChatService._build_sources(
        "MONEY_MOVES_ARTICLE", "why-diversify-101", None, resolved_context="article text",
    )
...
def test_malformed_citations_do_not_crash():
    ...
    sources = ChatService._build_sources("STOCK", "TSLA", citations, grounded=True)
...
def test_sources_capped_for_compact_card():
    ...
    sources = ChatService._build_sources("STOCK", "NVDA", citations, grounded=True)
...
# `test_other_contexts_still_emit_without_a_resolved_context` was retired 2026-09-11: the
# earned-pill rule is universal now (see test_an_ungrounded_screen_earns_no_pill below).

# ── EVERY context: the pill must be EARNED, not asserted (2026-09-11) ─────────────────


@pytest.mark.parametrize("ctype,ref", [
    ("TICKER_REPORT", "AAPL|warren_buffett"), ("STOCK", "MSFT"), ("ETF", "SPY"),
    ("CRYPTO", "BTCUSD"), ("INDEX", "^GSPC"), ("COMMODITY", "GCUSD"),
    ("MONEY_MOVES_ARTICLE", "why-diversify-101"), ("JOURNEY_LESSON", "3"),
])
def test_an_ungrounded_screen_earns_no_pill(ctype, ref):
    """TICKER_REPORT was the sharpest case: iOS sends no context for it, the resolver returns
    None on a cache miss, the chat answers from the live quote — and the card said
    'Cay research report · AAPL'. The resolver's 4 s timeout does the same on every
    detail screen. A context TYPE never earns the pill; arriving text (or an explicit
    `grounded=True` from the caller who knows) does."""
    assert ChatService._build_sources(ctype, ref, None) == []
    assert ChatService._build_sources(ctype, ref, None, resolved_context="   ") == []
    assert ChatService._build_sources(ctype, ref, None, grounded=False) == []
    earned = ChatService._build_sources(ctype, ref, None, resolved_context="real grounding text")
    assert len(earned) == 1 and earned[0]["label"] == ChatService._CONTEXT_SOURCE_LABEL[ctype]


def test_grounded_flag_overrides_the_text_heuristic():
    """A STOCK chat with server-side enrichment but no client snapshot is grounded."""
    assert ChatService._build_sources("STOCK", "MSFT", None, resolved_context=None, grounded=True)
    assert ChatService._build_sources("STOCK", "MSFT", None, resolved_context="text", grounded=False) == []
