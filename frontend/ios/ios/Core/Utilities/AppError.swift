...
enum AppError: Error, Identifiable, Equatable, Sendable {
    ...
    private static func mapAPIError(_ error: APIError) -> AppError {
        ...
        case .businessError(let code, let message):
            ...
            // AVATAR_UPLOAD_FAILED deliberately has NO branch here: `.apiError` surfaces the
            // backend's photo-specific `user_message` ("We couldn't save your photo right
            // now…") and offers `.retry` — the same photo works on the next attempt. A
            // `.serverError(503)` mapping existed briefly on the false premise that
            // `.apiError` "offers no action"; it replaced that copy with the generic
            // "technical difficulties" alert. The `.authError` arm above documents the
            // same decision.
