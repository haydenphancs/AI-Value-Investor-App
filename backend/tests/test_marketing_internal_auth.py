...
def test_every_route_in_the_module_is_gated(client, token, fake_service):
    """The gate lives on the router; nothing here answers without the header."""
    probes = [
        ("POST", f"{_BASE}/runs/claim", {"run_date": "2026-09-17", "worker_version": "t"}),
        ("PATCH", f"{_BASE}/runs/r1", {"stage": "selected"}),
        ("POST", f"{_BASE}/runs/r1/assets", {"kind": "manifest", "ext": "json", "sha256": "a" * 64, "bytes": 1}),
        ("POST", f"{_BASE}/assets/a1/complete", None),
        ("POST", f"{_BASE}/runs/r1/posts", {"posts": [{"platform": "x", "format": "text"}]}),
    ]
...
def test_claim_round_trip_and_extra_columns_are_ignored(client, token, fake_service):
    r = client.post(f"{_BASE}/runs/claim",
                    json={"run_date": "2026-09-17", "worker_version": "phase1", "dry_run": False},
                    headers={"X-Marketing-Worker-Token": token})
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["claimed"] is True and body["reason"] == "claimed"
    assert body["run"]["run_date"] == "2026-09-17" and body["run"]["dry_run"] is False
    assert "an_extra_column" not in body["run"]
    assert fake_service.calls == [("claim", date(2026, 9, 17), "phase1", False)]
...
def test_resume_only_claim_with_nothing_to_resume_returns_no_run(client, token, fake_service):
    r = client.post(f"{_BASE}/runs/claim",
                    json={"run_date": "2026-09-16", "worker_version": "t", "resume_only": True},
                    headers={"X-Marketing-Worker-Token": token})
    assert r.status_code == 200, r.text
