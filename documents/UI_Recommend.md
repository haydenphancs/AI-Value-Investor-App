...
4.  **Tracking** (Assets & Whales)
5.  **Wiser** (Education & Community)

**Top Navigation:**
- **Profile Icon:** Top-right corner (universally accessible)
- **Search Bar:** Top-left/center (context-dependent)
---
...
### 2. **Home: Dashboard / Home Screen**

**Data to Display:**
- **Market Tickers Row:** Live price + % change for S&P 500, Nasdaq, Bitcoin, Gold.
- **Market Insights Card:** (from `GET /api/v1/news/summary/latest`)
  - Headline, Bullet points (2 max), Sentiment tag (Bullish/Bearish).
- **Daily Briefing List:**
  - **Whales Alert:** e.g., "Large crypto whale moved $50M..."
  - **Earnings Alert:** e.g., "NVDA reports earnings tomorrow."
  - **Wiser Trending:** e.g., "How can I invest in OpenAI?"
- **Recent Research Carousel:** Horizontal scroll of recent reports generated by the user.

**User Actions:**
- Tap "See All" on Insights → navigates to **Updates** tab.
- Tap Alert → navigates to Detail view (Whale detail or Stock detail).
- Tap Research Card → navigates to **Research Report Detail**.
- Tap "New Analysis" button → navigates to **Research** tab.
**States:**
...
### 3. **Updates: News Feed Screen**

**Data to Display:**
- **Tab Switcher:** "Market" vs. [Watchlist Tickers e.g., "AAPL", "TSLA"]
- **Insight Summary Card:**
  - 24h AI Summary text specific to the selected tab.
  - "Updated [x] hours ago".
- **Live News Timeline:**
  - Vertical list with timestamps (e.g., "2:45 PM").
  - Source logo, Headline, Sentiment Badge (Positive/Neutral/Negative).
  - Thumbnail image.

**User Actions:**
- Switch Tab → triggers `GET /api/v1/news/feed?filter={tab}`.
- Tap Filter Icon → Open advanced filters (Source, Sector).
- Tap News Item → Open News Detail.
...
### 5. **Research: Stock ReSearch Screen**
...
- **Report Cards:**
  - Stock Ticker & Name.
  - Persona used (Icon + Name).
  - **Status Badge:** "Processing" (Yellow), "Ready" (Green), "Failed [Refunded]" (Red).
  - Date.
  - User Rating (Star system).
- **Community Insights:**
  - Feed of comments from other users regarding specific reports.
...
### 15. **Tracking Screen**
**Reference:** `Tracking_ Assets tab.jpg` & `Tracking_ Whales tab.jpg`

**UI Structure:** Top Segmented Control: **Assets** | **Whales**

#### **Tab A: Assets (Watchlist)**
**Data:**
- **Portfolio Insights Widget:** "Diversification Score" (Sector breakdown progress bar).
- **Asset List:** Ticker, Name, **Sparkline Chart** (7-day), Price, % Change.
- **Alerts & Events:**
  - Earnings Consensus (Beat/Miss).
  - Smart Money Following (e.g., "3 hedge funds bought...").

#### **Tab B: Whales (Institutional)**
**Data:**
- **Category Filter:** Following, Hedge Funds, Politicians, Crypto Whales.
- **Activity Feed:**
  - Entity Avatar (e.g., Buffett, Pelosi).
  - Action Badge (BOUGHT/SOLD).
  - Asset Ticker + Amount ($2.4B).
  - Source (13F Filing / Congressional Disclosure).
- **Trending/Popular:** List of entities to follow.

**User Actions:**
- Tap "Add New" → Search stock to add.
- Tap Whale Card → View detailed transaction history for that entity.
- Tap "Follow" button → Subscribes to entity alerts.
---

### 16. **Education Content Detail Screen**
...
### 17. **User Profile / Settings Screen**
...
**Tabs:**
1. Home (home icon)
2. Updates (newspaper icon)
3. Research (magnifying glass)
4. Tracking (book icon)
5. Wiser (person icon)
